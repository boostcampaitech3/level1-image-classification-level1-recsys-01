...
from loss import create_criterion, create_deit_criterion
...
def train_deit(data_dir, model_dir, args):
    ...
    # -- loss & metric
    base_model = load_model('/opt/ml/code/v2/model/exp2',18,device)
    base_model.to(device)
    base_cri = torch.nn.CrossEntropyLoss()

    criterion = create_deit_criterion(args.criterion, base_cri, base_model,'soft')  # train_loss
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            outs = model(inputs)
            loss = criterion(inputs,outs,labels)
            preds = torch.argmax((outs[0] + outs[1])/2, dim=-1)
            ...
            train_f1s += f1_score(preds.to('cpu').numpy(), labels.to('cpu').numpy(), average='macro')
            if (idx + 1) % args.log_interval == 0:
                train_loss = loss_value / args.log_interval
                train_acc = matches / args.batch_size / args.log_interval
                train_f1 = train_f1s / (idx+1)
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                val_f1_scores += f1_score(preds.to('cpu').numpy(), labels.to('cpu').numpy(), average='macro')
            ...
            val_f1 = val_f1_scores / len(val_loader)
            ...
            print(
                ...
                f"best acc : {best_val_acc:4.2%}, best f1 : {best_val_f1:4.2%} "
...
def train_by_f1score(data_dir, model_dir, args):
    seed_everything(args.seed)
...
if __name__ == '__main__':
    ...
    train_by_f1score(data_dir, model_dir, args)

    # train_deit(data_dir, model_dir, args)
