...
def train_by_f1score(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            train_f1s += f1_score(preds.to('cpu').numpy(),
                                  labels.to('cpu').numpy(), average='macro')
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                val_f1_scores += f1_score(preds.to('cpu').numpy(),
                                          labels.to('cpu').numpy(), average='macro')
            ...
            # best f1이 갱신될 때마다 저장할 것인지 best accuracy가 갱신될 때마다 저장될 것인지 선택
            # 선택한 것의 주석을 풀어준다
...
if __name__ == '__main__':
    ...
    parser.add_argument("--resize", nargs="+", type=list,
                        default=[224, 224], help='resize size for image when training')
    ...
    # 둘 중 필요한 것을 선택 (사용하지 않는 것은 주석 처리)
