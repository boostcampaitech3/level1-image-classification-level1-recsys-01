...
class RandAugmentation:
    ...
    def _apply_augment(self, img: Image, name: str, level: int) -> Image:
        ...
        augment_fn, lw, high = self.transforms[name]
        return augment_fn(img.copy(), level * (high - low) / self.n_level + low)

    def __call__(self, img: Image) -> Image:
        chosen_transforms = random.sample(self.transforms, k=self.n_level)
...
'''
class BaseAugmentation: # 512, 384
    def __init__(self, resize, mean, std, **args):
        ...
        # CenterCrop((int(482), int(354))), AddGaussianNoise(),
